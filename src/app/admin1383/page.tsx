...
const sections = [
  ...
  { href: "/admin/contact", label: "مدیریت تماس با ما", description: "ویرایش اطلاعات تماس", icon: "📞" },
];
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <Card className="p-6">
          <p className="text-sm text-muted">کل محصولات</p>
          <p className="text-3xl font-bold mt-1">{stats.totalProducts.toLocaleString("fa-IR")}</p>
        </Card>
        <Card className="p-6">
          <p className="text-sm text-muted">کل سفارش‌ها</p>
          <p className="text-3xl font-bold mt-1">{stats.totalOrders.toLocaleString("fa-IR")}</p>
        </Card>
        <Card className="p-6">
          <p className="text-sm text-muted">کل کاربران</p>
          <p className="text-3xl font-bold mt-1">{stats.totalUsers.toLocaleString("fa-IR")}</p>
        </Card>
      </div>
              ...
              <tbody>
                {stats.recentOrders.map((order) => (
                  <tr key={order.id} className="border-b border-border/30">
                    ...
                    <td className="py-3 text-end font-medium">{formatPrice(order.total)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
